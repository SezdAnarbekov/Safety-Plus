...
public class JoystickService extends Service {
    ...
    private void saveToDownloads() {
        ...
        DownloadManager downloadManager = (DownloadManager) JoystickService.this.getSystemService(DOWNLOAD_SERVICE);
        if (downloadManager != null) {
            downloadManager.addCompletedDownload(dir.getName(), dir.getName(), true, "video/3gpp", dir.getAbsolutePath(), dir.length(), true);
        }
    }
    ...
    private void sendMessage() {
        ...
        try {
            ...
            if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION)
                ...
                sendMessageWithLocation(smsBody);
            } else {
    ...
    private void sendMessageWithLocation(StringBuilder smsBody) {
        try {
    ...
    final class TheThread extends Thread {
        ...
        @Override
        public void run() {
            if (ActivityCompat.checkSelfPermission(JoystickService.this, Manifest.permission.RECORD_AUDIO)
                    != PackageManager.PERMISSION_GRANTED && ActivityCompat.
                    checkSelfPermission(JoystickService.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                    != PackageManager.PERMISSION_GRANTED) {
                return;
            } else {
                startRecording();
            ...
            while (flag) {
                try {
                    if (ActivityCompat.checkSelfPermission(JoystickService.this, Manifest.permission.SEND_SMS)
                            != PackageManager.PERMISSION_GRANTED) {
                        flag = false;
                    } else {
                        sendMessage();
        ...
        private void startRecording() {
            Date date = new Date();
            outputFile = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath() + "/" + date.toString() + "sp_audiorecords.3gp";
